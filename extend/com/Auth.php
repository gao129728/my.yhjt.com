...
class Auth{
    ...
    //默认配置
    /*protected $_config1   = array(
        'auth_on'           => true,                   // 认证开关
        'auth_type'         => 1,                      // 认证方式，1为实时认证；2为登录认证。
        'auth_group'        => 'one_auth_group',        // 用户组数据表名
        'auth_group_access' => 'one_auth_group_access', // 用户-用户组关系表
        'auth_rule'         => 'one_auth_rule',         // 权限规则表
        'auth_user'         => 'one_admin'             // 用户信息表
    );*/
    ...
    public function __construct() {
        ...
        $this->_config = array(
            ...
            'auth_user'         => config('database.prefix').'admin'             // 用户信息表
        );
    ...
    public function getGroups($uid) {
        ...
        $user_groups = \think\Db::name('auth_group_access')
            ->alias('a')
            ->join(config('database.prefix')."auth_group g", "g.id=a.group_id")
